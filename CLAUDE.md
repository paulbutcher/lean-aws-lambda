...
Three packages, and the split between the first two is the point of the library:

- `awsLambda` (root, library `AwsLambda`): the Lambda runtime API. JSON in, JSON out, any trigger.
- `awsLambda-http` (`http/`, library `AwsLambdaHttp`, namespace `AwsLambda.Http`): API Gateway
  payload format 2.0 mapped onto `Std.Http`.
- `awsLambda-tests` (`test/`): the suite for both.

**`AwsLambda` must never depend on `Std.Http` or on middleware.** A function serving a queue, a
schedule or a bucket notification imports only that layer, and the whole reason it is a separate
package is that such a function should not have to build an HTTP stack. Anything HTTP-shaped belongs
in `http/`. CI checks this, but the check only catches imports, not a design that has drifted.

The module root under `http/` is `AwsLambdaHttp` rather than `AwsLambda.Http` because Lake resolves
a module name to exactly one package. The Lean namespace is still `AwsLambda.Http`; don't "fix" the
...
- Before considering a task complete, run `lake build` in the repo root and in `http/`, and
  `lake test` from the repo root, as the final ground truth. A root `lake build` covers only the
  runtime layer.
...
- Neither shipping library may acquire a native dependency. Requiring a C toolchain or a system
  library would be inherited by every consumer, including the ones the layering exists to keep
  small.
...
- This is a published library, so a change to a public signature, a default, or the set of
  supported payload formats is a change to every consumer. Say so when proposing one.
...
- Time-box the attempt. If a proof will not close in a few tries, fall back to a
  ...
  weighed and why it failed. Never leave `sorry` or an admitted goal.
...
- A test that exercises the wire protocol must not depend on how the network happens to split a
  write. Loopback coalesces sends, so a response written in pieces is often read in one go, and a
  test built that way passes whether or not the code under test can resume from a partial read.
  Force the behaviour by size, past the reader's own buffer, instead. Rejecting chunked transfer
  encoding reached production precisely because nothing local ever produced it.
